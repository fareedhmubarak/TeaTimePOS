    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
                <h1 className="text-xl font-bold text-gray-800">Manage Expense Items</h1>
                            ...
                            {expenseItems.map(expense => (
                                <tr key={expense.id} className="bg-white border-b hover:bg-gray-50">
                                        ...
                                        {expense.allowSubItems && expense.subItems && expense.subItems.length > 0 && (
                                            <div className="mt-2 flex flex-wrap gap-1">
                                                {expense.subItems.map(subItem => (
                                                    <span key={subItem} className="px-2 py-0.5 text-sm font-normal bg-gray-200 text-gray-700 rounded-full">
                                                        {subItem}
                                                    </span>
                                                ))}
                                            </div>
                                    ...
                                    <td className="px-6 py-4 align-top">
                                        <span className={`px-2 inline-flex text-sm leading-5 font-semibold rounded-full ${expense.category === 'Monthly' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                                            {expense.category}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 align-top">
                             ...
                             {expenseItems.length === 0 && (
                                <tr><td colSpan={5} className="text-center py-8 text-gray-500 text-base">No expense items defined.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
